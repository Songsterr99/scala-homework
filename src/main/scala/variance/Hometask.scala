...
object Hometask extends App {
  ...
  object Matrix {
    ...
    implicit val comonad: Comonad[Matrix] = new Comonad[Matrix] {
      ...
      override def coflatMap[A, B](fa: Matrix[A])(f: Matrix[A] => B): Matrix[B] = {
        val res = for {
          y <- fa.matrix.indices
          x <- fa.matrix(0).indices.reverse
        } yield f(new Matrix(fa.matrix, Focus.of(x, y).getOrElse(Focus.Zero)) {})

        new Matrix(res.sliding(res.size / fa.matrix.size, res.size / fa.matrix.size).map(_.toVector).toVector, Focus.Zero) {}
      }
  ...
  writeImage(smoothed, "smoothed2.bmp")
 // writeImage(mirrored, "mirrored2.bmp")
